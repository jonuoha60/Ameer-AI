...
func NewService(repo *Repo, jwtRefresh, jwtAccess string, db *mongo.Database, fireBaseCred string) *Service {
	return &Service{repo: repo, tokenRepo: refresh.NewRepo(db), jwtRefresh: jwtRefresh, jwtAccess: jwtAccess, fireBaseCred: fireBaseCred}
}
...
func (s *Service) GoogleSignup(ctx context.Context, token string) (AuthResponse, error) {
	user, err := s.repo.GoogleCreate(ctx, token, s.fireBaseCred)
	if err != nil {
		return AuthResponse{}, err
	}
